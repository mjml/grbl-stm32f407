/*
  settings.c - flash configuration handling
  Part of Grbl
...
const unsigned char flash_data_area[0x4000];
unsigned int flash_data_ptr = 0;

// Method to store startup lines into FLASH
void settings_store_startup_line(uint8_t n, char *line) {
#ifdef FORCE_BUFFER_SYNC_DURING_FLASH_WRITE
  protocol_buffer_synchronize();  // A startup line may contain a motion and be
                                  // executing.
#endif
  uint32_t addr = n * (LINE_BUFFER_SIZE + 1) + FLASH_ADDR_STARTUP_BLOCK;
  memcpy_to_flash_with_checksum(addr, (char *)line, LINE_BUFFER_SIZE);
}

// Method to store build info into FLASH
// NOTE: This function can only be called in IDLE state.
void settings_store_build_info(char *line) {
  // Build info can only be stored when state is IDLE.
  memcpy_to_flash_with_checksum(FLASH_ADDR_BUILD_INFO, (char *)line,
                                LINE_BUFFER_SIZE);
}

// Method to store coord data parameters into FLASH
void settings_write_coord_data(uint8_t coord_select, float *coord_data) {
#ifdef FORCE_BUFFER_SYNC_DURING_FLASH_WRITE
  protocol_buffer_synchronize();
#endif
  uint32_t addr =
      coord_select * (sizeof(float) * N_AXIS + 1) + FLASH_ADDR_PARAMETERS;
  memcpy_to_flash_with_checksum(addr, (char *)coord_data,
                                sizeof(float) * N_AXIS);
}

// Method to store Grbl global settings struct and version number into FLASH
// NOTE: This function can only be called in IDLE state.
void write_global_settings() {
  flash_put_char(FLASH_ADDR, SETTINGS_VERSION);
  memcpy_to_flash_with_checksum(FLASH_ADDR_GLOBAL, (char *)&settings,
                                sizeof(settings_t));
}

// Method to restore FLASH-saved Grbl global settings back to defaults.
...
#if N_STARTUP_LINE > 0
    flash_put_char(FLASH_ADDR_STARTUP_BLOCK, 0);
    flash_put_char(FLASH_ADDR_STARTUP_BLOCK + 1, 0);  // Checksum
#endif
#if N_STARTUP_LINE > 1
    flash_put_char(FLASH_ADDR_STARTUP_BLOCK + (LINE_BUFFER_SIZE + 1), 0);
    flash_put_char(FLASH_ADDR_STARTUP_BLOCK + (LINE_BUFFER_SIZE + 2),
                   0);  // Checksum
#endif
  }

  if (restore_flag & SETTINGS_RESTORE_BUILD_INFO) {
    flash_put_char(FLASH_ADDR_BUILD_INFO, 0);
    flash_put_char(FLASH_ADDR_BUILD_INFO + 1, 0);  // Checksum
  }
}
...
uint8_t settings_read_startup_line(uint8_t n, char *line) {
  uint32_t addr = n * (LINE_BUFFER_SIZE + 1) + FLASH_ADDR_STARTUP_BLOCK;
  if (!(memcpy_from_flash_with_checksum((char *)line, addr,
                                        LINE_BUFFER_SIZE))) {
...
uint8_t settings_read_build_info(char *line) {
  if (!(memcpy_from_flash_with_checksum((char *)line, FLASH_ADDR_BUILD_INFO,
                                        LINE_BUFFER_SIZE))) {
    // Reset line with default value
    line[0] = 0;  // Empty line
...
uint8_t settings_read_coord_data(uint8_t coord_select, float *coord_data) {
  uint32_t addr =
      coord_select * (sizeof(float) * N_AXIS + 1) + FLASH_ADDR_PARAMETERS;
  if (!(memcpy_from_flash_with_checksum((char *)coord_data, addr,
                                        sizeof(float) * N_AXIS))) {
...
uint8_t read_global_settings() {
  ...
  if (version == SETTINGS_VERSION) {
    ...
    if (!(memcpy_from_flash_with_checksum((char *)&settings, FLASH_ADDR_GLOBAL,
                                          sizeof(settings_t)))) {
      return (false);
    }
  } else {
  ...
  if (parameter >= AXIS_SETTINGS_START_VAL) {
    ...
    switch (parameter) {
      ...
      case 2:
        ...
        st_generate_step_dir_invert_masks();  // Regenerate step and direction
                                              // port invert masks.
        break;
      case 3:
...
void settings_init() {
  flash_data_ptr = (unsigned int)flash_data_area;
  if (!read_global_settings()) {
    report_status_message(STATUS_SETTING_READ_FAIL);
    settings_restore(SETTINGS_RESTORE_ALL);  // Force restore all FLASH data.
